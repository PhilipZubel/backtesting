...
def plot_candlestick_with_indicators(df: pd.DataFrame, line_names: dict[str, str], subplot_lines: dict) -> go.Figure:
    ...
    fig = make_subplots(
        rows=2 if subplot_lines else 1, cols=1,
        shared_xaxes=True,
    ...
    for idx in position_change_indices:
        fig.add_vline(x=df['Date'].iloc[idx], line=dict(color='gray', dash='dash', width=1), row=1, col=1)

    colors = ['orange', 'blue', 'yellow', 'purple', 'brown']
    if subplot_lines:
        horizontal_lines = {k: v for k, v in subplot_lines.items() if isinstance(v, (int, float))}
        normal_lines = {k: v for k, v in subplot_lines.items() if not isinstance(v, (int, float))}
        for line_name, column_name in normal_lines.items():
            fig.add_trace(go.Scatter(
                x=df['Date'], y=df[column_name],
                mode='lines', name=line_name, line=dict(color=colors.pop(0))
            ), row=2, col=1)
