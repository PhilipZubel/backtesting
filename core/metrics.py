...
def track_positions(df):
    position_changes = df[
        df["Position"] != 0
    ]  # Filter to include only non-zero positions
    ...
    for i in range(len(position_changes)):
        ...
        if row["Position"] == 1:  # A buy signal (long position)
            ...
            row["Position"] == -1 and entry_row is not None
        ):  # A sell signal (closing the position)
            exit_row = row
            profit = (exit_row["Close"] - entry_row["Close"]) * entry_row[
                "Position"
            ]  # Calculate profit
